...
Scalar.from_bytes = function (buffer) {
  if (
    buffer.length != Scalar.expected_byte_length() &&
    buffer.length != 2 * Scalar.expected_byte_length()
  ) {
...
GroupElement.expected_byte_length = function (
  curve /* Curve */,
  is_compressed
) {
  if (typeof curve == "undefined") {
...
GroupElement.generate_random = function (curve) {
  ...
  return new GroupElement(
    curve.generator().mul(Scalar.generate_random().valueOf())
  );
};
...
GroupElement.from_bytes = function (buffer) {
  ...
  if (buffer.length != ge_size) {
    throw new Error(
      `Invalid length of data. expected: ${ge_size} - actual: ${buffer.length}`
    );
  }
...
GroupElement.prototype.add = function (ge /* GroupElement */) {
  return new GroupElement(this.valueOf().add(ge.valueOf()));
};
GroupElement.prototype.mul = function (sc /* Scalar */) {
  return new GroupElement(this.valueOf().mul(sc.valueOf()));
};
GroupElement.prototype.eq = function (ge /* GroupElement */) {
...
function PrivateKey(prvKey /* Scalar */, pubKey /* PublicKey */) {
  ...
  if (typeof pubKey == "undefined") {
    ...
    pubKey = new PublicKey(
      new GroupElement(curve.generator().mul(prvKey.valueOf()))
    );
  }
...
PrivateKey.generate = function (seed, curve, options) {
  ...
  return new PrivateKey(
    new Scalar(kp.getPrivate()),
    new PublicKey(new GroupElement(kp.getPublic()))
  );
};
...
PrivateKey.prototype.get_public_key = function () {
  ...
  return new PublicKey(
    new GroupElement(curve.generator().mul(this.valueOf().valueOf()))
  );
};
 ...
 */
function ReEncryptionKey(
  re_key /* Scalar */,
  internal_public_key /* GroupElement */
) {
...
Capsule.from_bytes = function (buffer) {
  ...
  var XG = undefined;
  if (re_encrypted) {
    XG = GroupElement.from_bytes(
      buffer.slice(2 * ge_size + sc_size, 3 * ge_size + sc_size)
    );
  }
...
Proxy.generate_key_pair = function (seed) {
...
Proxy.decapsulate_re_encrypted = function (capsule, privateKey) {
  ...
  var points_for_hash = [
    prime_XG,
    privateKey.get_public_key().valueOf(),
    prime_XG.mul(privateKey.valueOf())
  ];
  var hash = hash_to_scalar(points_for_hash);
...
Proxy.private_key_from_bytes = function (data) {
  return PrivateKey.from_bytes(data);
};
Proxy.public_key_from_bytes = function (data) {
  return PublicKey.from_bytes(data);
};
Proxy.re_encryption_key_from_bytes = function (data) {
  return ReEncryptionKey.from_bytes(data);
};
Proxy.capsule_from_bytes = function (data) {
  return Capsule.from_bytes(data);
};
Proxy.to_hex = function (data) {
  return to_hex(data);
};
Proxy.from_hex = function (data) {
